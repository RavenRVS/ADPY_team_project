class Bot_VK:
    def __init__(self):
        ...
        # 3)     подключение к БД
        # 4)     таблицы SQL
        # 5)     Стартовые параметры клавиатуры
    ...
    def get_param_for_searching(self, возраст, пол, город):
        ...
        # 2)     На выходе (возраст от, возраст до, пол противоположный пользователю, город).
        # Если какой-либо из параметров не получен на вход (отсутствуют в профиле), то устанавливает значения
        # по умолчанию
    ...
    def hi_command(self):
        pass
        # 1)     через метод get_info_about_user класса request_VK получаем имя, возраст, город, пол пользователя
        # 2)     методом check_user проверяем есть ли пользователь в БД, если нет, то добавляем в базу
        # (через метод insert_user модуля SQL_function записываем данные в БД,), если есть – проверяем актуальность
        # имеющихся в базе данных (метод check_and_update_param_user модуля SQL_function)
    ...
    def next_challenger(self):
        ...
        # 2)     Методом search_people модуля request_VK получаем список ID подходящих пользователей
        # 3)     Через random выбираем из списка любого кандидата
        # 4)     Методом get_info_about_challenger модуля request_VK получаем информацию о выбранном пользователе
        # 5)     Методом get_link_challenger модуля request_VK получаем ссылку на страницу кандидата
        ...
        # 7)     Методом get_photo модуля request_VK получаем до трех фото найденного кандидата
    ...
    def get_favorite_list_command(self):
        ...
        # b)     Через метод get_photo модуля request_VK получаем до трех фото кандидата
